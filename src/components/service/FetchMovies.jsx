...
async function FetchCastApi({ params }) {
    console.log(params);
    const response = await axios.get(`${URL}movie/${params.movieId}/credits?api_key=${KEY}`);
    return response.data;
}
async function FetchReviewsApi({params}) {
    const response = await axios.get(`${URL}movie/${params.movieId}/reviews?api_key=${KEY}`);
    return response.data;
}
const api = {
    FetchHomeApi,
